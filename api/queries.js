...
//implement app.post(/loanpools)
    const postNewLoanPool = (request, response) => {
        ...
        pool.query("INSERT INTO loans (duration, principal, interest) VALUES $1, $2, $3", [duration, principal, interest], (error, results) => {
            if (error) {
                throw error;
            }
...
//implement app.put(/loanpools/:id)
    const updateLoanInPool = (request, response) => {
        ...
        const {duration, principal, interest} = request.body;
        pool.query("UPDATE loans SET duration = $1, principal = $2, interest = $3 WHERE id = $4", [duration, principal, interest, id], (error, results) => {
            if (error) {
                throw error;
            }
